...
- CPU-only FM and digital-TV service-contour reference plotting from bundled quantized P.1546-6 land tables, with protected, explicitly revoked E(50,10) legacy interfering envelopes, statistical-screening, incomplete, and `NoData` states kept distinct, plus deterministic SAF KMZ export of already calculated contour evidence;
- a CPU-only FM and digital-TV regulatory study that keeps the project transmitter independent, derives the applicable P.1546 protected contour, discovers nationwide cochannel/first-adjacent Basic Plan candidates, evaluates P.526-15 Deygout-Assis D/U in both directions, applies the digital-TV colocated-adjacent ERP rule, and retains point evidence plus explicit filing gates;
...
This release does **not** claim to deliver an authorized offline basemap package, RF clutter loss, a building-height model, full-wave or measured 3D antenna modeling, `.atxp`/`.rp3` project import, an Anatel licensing/station-authority conclusion, a current regulatory interfering iso-field contour, or complete desktop parity. The red dash-dot E(50,10) envelopes are reconstructed from revoked rules and never labeled current regulatory results. The current FM/digital-TV workflow now uses on-demand ANADEM v1.0 DTM ranges, official IBGE 2022 urban geometry, and a verified MCom licensed baseline with explicit existing-versus-proposed status; analog TV/RTV and former TV channels 5/6 are outside product scope. It remains not filing-ready until independent numerical parity and qualified professional/legal/source-license review are completed. Basic Plan rows are read-only viability/interference references and never populate the independently created project transmitter. See [Android regulatory input pipeline](docs/REGULATORY_INPUT_PIPELINE.md) and [Antenna, coverage, interference, and export parity](docs/ANTENNA_COVERAGE_PARITY.md).
...
The broadcast slice follows the pinned protected-contour profiles: FM `E(50,50)` at 66 dBµV/m and digital TV `E(50,90) = 2 × E(50,50) − E(50,10)` at 43 dBµV/m for channels 7–13 or 51 dBµV/m for channels 14–51. Extended FM is represented by channels 141–197 at 76.1–87.3 MHz; former analog-TV channels 5 and 6 are never treated as TV projects. FM/TV cochannel and first-adjacent E(50,10) envelopes are available only as clearly labeled non-regulatory reconstructions of revoked rules. `E(80,80)` is unsupported and remains `NoData`. The current P.526/Assis study evaluates FM/FM and digital-TV/digital-TV pairs in both directions; missing terrain, station-class fallback, candidate capacity, external protected contour, or baseline evidence fails closed. See [Brazil broadcast service contours](docs/BRAZIL_BROADCAST_CONTOURS.md) for source versions, thresholds, assumptions, and remaining gates.
...
| Anatel Basic Plan catalog | explicit review-gated whole-archive HTTPS acquisition, immutable raw snapshots, bounded ZIP/XML parsing, staged/atomic SQLite v1, offline core/UI queries, and transient read-only broadcast-study reference integration; no bundle, background/process survival, metadata resolution, or durable project/snapshot pinning and diff |
...
The current physical-device instrumentation also exercises 360 × 480 dp layouts at font scale 1.30 for Manage RF Assets, Data Catalog, Engineering Map, Antenna Pattern Lab, and Studies. The Studies coverage includes a dedicated independent FM project that keeps the P.526-15 Deygout-Assis, E(50,50), nationwide Basic Plan, bidirectional D/U, radius, and run controls reachable without reducing the system font scale. Earlier emulator observations remain historical evidence only and are no longer the active validation path.

A bounded historical force-stop/relaunch check retained the archived record. Restoring it retained the project as active and selected across a second force-stop/relaunch. This is evidence for the tested local catalog path only; it is not proof of Android Backup, system-reclaim restoration, every process-death timing, or a broader device support matrix.
...
The physical-device observations and historical lab checks are bounded evidence, not a support matrix. They do not establish coverage for other OEMs, Android versions, aspect ratios, tablets, foldables, extreme font scales, screen readers, switch access, or every light/dark-theme combination. Those checks remain release work.
...
Current instrumented validation requires a connected physical Android device:

```powershell
.\gradlew.bat connectedDebugAndroidTest
```

The current aggregate instrumented run passed **100/100 tests** with no failures or skips on the physical Android 16/API 36 reference phone (`25080RABDG`). It includes navigation, Activity recreation, compact Compose screens, structured and arbitrary antenna-composer controls, the independent FM current-rule study at 360 × 480 dp/font scale 1.30, legacy-contour map semantics, coverage render-mode/`NoData` reachability at enlarged text, Anatel/IBGE repositories, project persistence, and regional-job/WorkManager foundations. API 23/33 runtime proof and a broader physical-device matrix remain open.

The current aggregate JVM run discovered **616 tests**: **615 passed**, one Windows symlink-hardening case was permission-skipped, and there were zero failures or errors across project persistence, acquisition, durable jobs, WorkManager contracts, antenna composition/interchange/identity, current FM/DTV protected contours and bidirectional D/U, coverage palettes/surfaces, HTML/PDF/XLSX export, Anatel, IBGE, and RF-domain coverage. The physical Android 16/API 36 aggregate passed **100/100** tests with no failures or skips. `lintDebug` completed with 0 errors, 19 warnings, and 3 hints; debug APK and Android-test APK assembly also succeeded. A dated official-source acquisition downloaded the 11,870,186-byte Anatel archive with SHA-256 `51391ba6d2c9a58233eeedd8cc0fef64eb4ab8f33622b9ea6e469d7bf90384f6`, parsed and indexed all 87,400 emitted TV/FM source records, and verified offline FM/TV query, source details, and replacing 25-record pagination. This is one provider snapshot, not a provider-availability, licensing, or regulatory-validity guarantee. Separately, a bounded live-provider run downloaded 65,317 raw OSM bytes through the fixed Lambert route, produced 44 building/building-part way features, verified both raw and processed SHA-256 values, retained the actual endpoint and upstream OSM timestamp, and reopened its schema-2 inventory. That OSM run is smoke evidence for one small request, not a completeness guarantee; the delivered durable adapter remains unreachable from the current Data screen, and API 23/33, UIDT, late network-byte/checkpoint crash, process-death, and reboot proof remain open.
