...
- a bounded Antenna Pattern Lab with reviewed SAF import, deterministic two-file HRP/VRP pairing, immutable source/canonical artifacts, convention-explicit PRN, ADT HRP/VRP, V-Soft HRP/VRP, Progira PAT, native ATX Antenna JSON v2 interchange with legacy-v1 reading, strict ATX Planner desktop JSON v1 attenuation/phase import and export compatibility, explicit-plane generic CSV/TXT import, complex-vector duplicate-angle averaging, gain-bound normalized-content identity V2, interchange-safe canonical-artifact deduplication, converged CPU array synthesis with explicit separable-cut validity gates, a compact pattern library, sector assignment, and verified SAF export with surfaced format-loss warnings;
...
- CPU-only FM and first-generation digital-TV service-contour reference plotting from bundled quantized P.1546-6 land tables, with protected, statistical-screening, incomplete, and `NoData` states kept distinct, plus deterministic SAF KMZ export of already calculated contour evidence;
...
This release does **not** claim to deliver a basemap, bare-earth DTM, raster terrain sampler, RF clutter adapter, building-height model, full-wave or measured 3D antenna modeling, `.atxp`/`.rp3` import, raster coverage, census-sector polygons, population-by-coverage, an Anatel licensing/station-authority conclusion, or a strict normative propagation/interference workflow. Downloaded GeoTIFF files are metadata-indexed but not sampled by an engineering engine. Broadcast contours are visibly non-regulatory planning references: all geometry still uses sector AGL as an effective-height proxy. A calculation-ready assigned HRP can shape radial ERP; otherwise the planner discloses its nominal omnidirectional fallback. Basic Plan rows remain a separately acquired channel-planning catalog and are not applied to projects or contours. The **Data & capabilities** screen keeps those boundaries visible in the application.
...
The Engineering Map is a bounded offline geographic-coordinate tool, not a basemap claim. Pure Kotlin Web Mercator camera math drives project fitting, direct-manipulation pan/pinch zoom, antimeridian-aware projection, and a metric scale. Selecting a site exposes an accessible coordinate editor whose stale-safe transaction changes only latitude and longitude and publishes the new point only after durable storage succeeds. The map can also render locally calculated broadcast-contour references beneath sites: protected contours are solid, statistical screening is dashed, incomplete geometry is unfilled, and `NoData` is never drawn. When a sector has a calculation-ready assigned HRP, each radial uses the stored peak ERP multiplied by the interpolated field ratio squared; an absent cut uses the explicitly warned nominal fallback. Its KMZ action exports only these already calculated overlays in a deterministic `doc.kml` plus evidence-manifest package and verifies the SAF destination by reopening it; KML/KMZ is explicitly rejected as antenna-pattern input and is not treated as a regulatory filing. The screen permanently discloses that no third-party tiles, terrain, clutter, general GIS features, or raster coverage are rendered. A non-null elevation is labeled as a stored project value and is not resampled when coordinates move; a missing value remains explicit `NoData`.

The broadcast slice follows the currently pinned Brazilian protected-contour profiles: FM `E(50,50)` at 66 dBµV/m and first-generation digital TV `E(50,90) = 2 × E(50,50) − E(50,10)` at 43 dBµV/m for channels 7–13 or 51 dBµV/m for channels 14–51. FM `E(50,10)` is available only as a clearly labeled non-regulatory statistical screen. `E(80,80)` is unsupported and remains `NoData`. Current Anatel interference compliance still requires point-to-point P.526 plus Assis and D/U evaluation and is not implemented. See [Brazil broadcast service contours](docs/BRAZIL_BROADCAST_CONTOURS.md) for source versions, table identity, assumptions, and blocked strict work.
