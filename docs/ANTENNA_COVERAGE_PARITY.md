...
| Coherent array calculation | Delivered bounded slice | CPU-only complex-field sum supports up to 512 elements with per-element pattern, 3D wavelength position, relative power, feed phase plus delay, orientation, enable state, normalized excitation, bounded adaptive 3D integration, gain/directivity, HRP, and VRP. | Reusable editable configuration persistence and independent cross-engine numerical validation remain absent. |
| Forward synthesis UI | Delivered bounded slice | Single element, vertical stack, horizontal linear, planar, circular, outward-oriented multipanel, and compact arbitrary-element editing; wavelength spacing/radius; scan; uniform/cosine/binomial taper for structured arrays; project attachment; and artifact-correlated export. | Add polarization/grouping, Chebyshev taper, null fill, tower/feed effects, batch workflow, and saved design configurations. |
...
| Digital-TV protected contour | Delivered bounded regulatory calculation | Independent project transmitter, on-demand ANADEM v1.0 DTM, terrain-derived radial HNMT, P.1546-6 land tables, derived `E(50,90)`, 72 radials, explicit `NoData`, range-integrity evidence, and engineering gates. | Independent numerical parity and qualified professional/legal/source-license review remain external filing gates. |
| FM protected/statistical overlays | Delivered bounded regulatory/reference slices | The current-rule study derives the 72-radial `E(50,50)` protected contour at 66 dBµV/m with ANADEM terrain-derived HNMT and largest-crossing logic. A separate standalone `E(50,10)` screen remains explicitly revoked and non-regulatory; unsupported `E(80,80)` remains `NoData`. | Independent goldens and professional review remain filing gates; do not relabel the historical screen as current evidence. |
...
| Protected-boundary D/U | Delivered bounded FM/digital-TV same-service slice | FM/FM applies `+30 dB` cochannel and `+6 dB` first-adjacent criteria; digital-TV/digital-TV applies `+19 dB` and `-36 dB`. Every bounded calculation-ready candidate retains bidirectional desired/undesired fields, P.526-15 Deygout-Assis loss, margin, pass/fail/`NoData`, reference provenance, and map markers. Colocated adjacent digital channels use the ERP-ratio method in both directions. Analog TV/RTV and TV channels 5/6 are excluded by scope. | Independent normative vectors, area/grid D/U beyond the delivered urban gate, receiver-pattern exceptions, and other-service relations remain. Boundary dots are not an interfering iso-field contour. |
| Interfering iso-field contours | Delivered historical reference; current regulatory iso-field not claimed | Red dash-dot E(50,10) cochannel/first-adjacent envelopes reproduce threshold arithmetic from revoked FM Resolution 67/1998 and TV Resolution 398/2005, retain provenance, and set `regulatory = false`. Current FM and digital-TV evidence is the separate bidirectional P.526-15 Deygout-Assis protected-boundary workflow. | Area/grid D/U, normalized reference-station patterns, independent goldens, mixed-service rules, and any justified current-regulation iso-field product remain open. |
| Basic Plan channel use | Delivered bounded integration | Complete nationwide same-service cochannel/first-adjacent partitions are discovered as read-only viability/D/U references. The closest 48 calculation-ready records are evaluated; every excess applicable record is counted and blocks filing readiness. Project transmitter values remain independent. | Add immutable catalog snapshot pinning/diff, uncapped or safely partitioned execution, explicit TV generation/subtype, and review of every applicable record without copying catalog values into project-owned fields. |
...
4. Keep legacy E(50,10) envelopes labeled revoked and non-regulatory; never substitute them for current P.526/Assis D/U evidence.
