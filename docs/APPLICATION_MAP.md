...
4. create, select, rename, and transactionally duplicate local projects;
...
| Local build evidence | Delivered | Debug APK and test APK exist; latest lint report has 0 errors and 10 dependency/tooling warnings. | This is development evidence, not a signed release gate. |
...
| Adaptive UI | Delivered | Bottom navigation is used on compact widths and a navigation rail at 720 dp or wider. Rail labels/header collapse to five accessible icons below 520 dp height. Dashboard metrics, Studies fields/results, RF and project-name editor fields, the compact adaptive Duplicate Project dialog, the map canvas/site list, and Data cards use bounded responsive layouts. The Activity resizes for the IME so the short landscape project-name editor and duplication dialog retain reachable editable content and actions. | The shell and bounded feature density are adaptive; wider device coverage and full adaptive list/detail patterns still need testing. |
| Compact phone density | Delivered baseline | Scalable typography, 16 dp Dashboard/Projects gutters, 12 dp field-heavy screen gutters, compact shared components, responsive fields/cards/map height, and explicit 48 dp minimums on controls changed by the pass are implemented. Manual checks used one physical Android 16 phone at 1280 × 2772 pixels and 520 dpi: portrait at font scales 1.15 and 1.30, plus landscape at 1.15. Separate Android 16/API 36 emulator checks covered the Duplicate Project dialog at font scales 1.0 and 1.30, including short landscape with the IME open. | These are bounded physical-device and emulator observations, not a device, aspect-ratio, theme, font-scale, or accessibility matrix. No system font-scale override or clamp is used. |
...
| Project operations | Foundation | Load, create, select, rename, duplicate, and transactional mutation are delivered. Rename preserves project identity and its RF graph and rejects a stale competing name. Duplication reads the latest durable source inside the transaction, assigns a fresh root ID and root timestamps, preserves the complete project-scoped RF graph, nested IDs/references/data/order and study timestamps, leaves the source unchanged, appends the copy, and selects it. Add RF Path persists one linked network/site/sector/receiver without exposing partial state. | Delete, archive, import, independent RF-entity CRUD, and impact-aware linked deletion are not delivered. A duplicate does not yet record source-project lineage or a duplication-provenance marker. |
...
| Projects and nested editors | Foundation | Projects lists/selects/creates projects, opens a compact adaptive duplication dialog, opens a saveable rename editor that transactionally changes only the selected project name, and opens a saveable Add RF Path editor that validates and transactionally adds one selected-system RF network, transmitter site/sector, and receiver. | Rename and duplication are bounded project-operation slices and Add RF Path is one combined create slice, not complete project lifecycle or independent RF-entity CRUD; broader device testing remains. |
...
| JVM tests | Delivered baseline | The current 101-test JVM suite passes and covers model/value boundaries, references and JSON round trips, RF formulas, schema migration/storage faults/concurrency, transactional duplication, application use cases, form parsing, ViewModel transitions, and English-only source hygiene. | Property/numerical golden, accessibility, performance, export, and complete system-flow coverage remain. |
| Instrumented tests | Delivered baseline | The current 25-test suite passes on an Android 16/API 36 emulator, exercising supported, unknown, malformed, nested RF-path, and nested project-name route restoration; explicit mutation-completion and transient pending-save recovery; legacy-name normalization; competing-rename and draft-discard protection; accessibility semantics; persisted project rename; project-duplication draft restoration, rejection recovery, durable selected-copy persistence and Activity recreation; and create-project -> persist-RF-path -> Activity-recreation behavior. The duplication dialog also has manual emulator checks at font scales 1.0/1.30 in short landscape with the IME open. The preceding 18-test revision passed on the physical Android 16 reference phone. | A fresh physical run of the current suite, true process termination, broader accessibility automation, a formal device matrix, and CI execution remain. |
...
| Local project catalog | Delivered baseline | P0 | F0/F2 | Schema-2 transactional JSON loads, explicitly migrates v1, creates/selects/renames/duplicates, and persists catalog mutations. |
...
| Project lifecycle | Foundation | P0 | F2 | Rename and transactional duplication are delivered; add archive, delete, recovery, lineage/provenance policy, and consistency checks. |
