...
4. create, select, rename, transactionally duplicate, and transactionally hard-delete local projects;
...
| Adaptive UI | Delivered | Bottom navigation is used on compact widths and a navigation rail at 720 dp or wider. Rail labels/header collapse to five accessible icons below 520 dp height. Dashboard metrics, Studies fields/results, RF and project-name editor fields, the compact adaptive Duplicate Project and Delete Project dialogs, the map canvas/site list, and Data cards use bounded responsive layouts. The Activity resizes for the IME so the short landscape project-name editor and destructive dialogs retain reachable editable content and actions. | The shell and bounded feature density are adaptive; wider device coverage and full adaptive list/detail patterns still need testing. |
| Compact phone density | Delivered baseline | Scalable typography, 16 dp Dashboard/Projects gutters, 12 dp field-heavy screen gutters, compact shared components, responsive fields/cards/map height, and explicit 48 dp minimums on controls changed by the pass are implemented. Manual checks used one physical Android 16 phone at 1280 × 2772 pixels and 520 dpi: portrait at font scales 1.15 and 1.30, plus landscape at 1.15. Separate Android 16/API 36 emulator checks at 1080 × 2400 pixels and 420 dpi covered the Duplicate Project and Delete Project dialogs at font scales 1.0 and 1.30, in portrait and short landscape with Gboard open and closed. At 1.30, portrait stacked project actions; short landscape required one scroll to the delete field and exposed actions after hiding the IME. | These are bounded physical-device and emulator observations, not a device, aspect-ratio, theme, font-scale, or accessibility matrix. No system font-scale override or clamp is used. |
...
| Project operations | Foundation | Load, create, select, rename, duplicate, hard-delete, and transactional mutation are delivered. Rename rejects a stale competing name. Duplication reads the latest durable source, preserves the project-scoped graph, and selects a fresh-root copy. Hard deletion compares the complete reviewed aggregate with the latest durable version, rejects stale changes, removes the unchanged current catalog aggregate atomically, preserves every other project, and selects a deterministic valid neighbor or none. Add RF Path persists one linked network/site/sector/receiver without exposing partial state. | Archive, recovery/undo/export, import, project-owned asset policy, independent RF-entity CRUD, and impact-aware linked deletion are not delivered. A duplicate does not yet record source-project lineage or a duplication-provenance marker. |
...
| Projects and nested editors | Foundation | Projects lists/selects/creates projects, opens compact adaptive duplication and exact-keyword deletion dialogs, opens a saveable rename editor that transactionally changes only the selected project name, and opens a saveable Add RF Path editor that validates and transactionally adds one selected-system RF network, transmitter site/sector, and receiver. | Rename, duplication, and hard deletion are bounded project-operation slices and Add RF Path is one combined create slice, not complete project lifecycle or independent RF-entity CRUD; broader device testing remains. |
...
| JVM tests | Delivered baseline | The current 125-test JVM suite passes and covers model/value boundaries, references and JSON round trips, RF formulas, schema migration/storage faults/concurrency, transactional duplication, project-deletion selection/conflict/no-op policy, application use cases, form parsing, ViewModel transitions, and English-only source hygiene. | Property/numerical golden, accessibility, performance, export, and complete system-flow coverage remain. |
| Instrumented tests | Delivered baseline | The current 33-test suite passes on an Android 16/API 36 emulator, exercising supported, unknown, malformed, nested RF-path, and nested project-name route restoration; explicit mutation-completion and transient pending-save recovery; rename and duplication behavior; accessibility semantics; exact hard-delete confirmation, changed-snapshot and deletion draft/rejection/durable-absence restoration, deterministic fallback selection, and Activity recreation after deletion; and create-project -> persist-RF-path -> Activity-recreation behavior. Manual emulator checks also cover the Duplicate Project and Delete Project dialogs at font scales 1.0/1.30 in portrait and short landscape with Gboard open/closed. The preceding 18-test revision passed on the physical Android 16 reference phone. | A fresh physical run of the current suite, true process termination, broader accessibility automation, a formal device matrix, and CI execution remain. |
...
| **Projects** | Compact wrapping catalog cards, selection/create/duplication/deletion dialogs, schema/details, and entry to nested project-name and Add RF Path editors. | Foundation | Project archive/recovery, independent network/site/sector/receiver CRUD, and broader device/accessibility evidence. |
| **Rename Project** | Saveable compact name draft, explicit impact statement, transactional local save, durable-success return, normalized dirty-exit protection, and stale competing-rename rejection. | Delivered bounded slice | Metadata editing, archive/recovery, richer conflict diagnostics, and broader device/accessibility evidence. |
| **Duplicate Project** | Compact adaptive dialog with a saveable normalized name draft. The transaction copies the latest durable source aggregate, assigns a fresh root ID/timestamps, leaves the source unchanged, appends and selects the durable copy, and closes the dialog only after completion is observable. | Delivered bounded slice | Source-lineage/duplication provenance, archive/undo, and broader device/accessibility evidence. |
| **Delete Project** | Compact adaptive dialog reports current project-scoped counts and requires exact `DELETE`. The transaction structurally compares the reviewed aggregate with the latest durable version, rejects stale changes, atomically removes an unchanged target, preserves other projects, selects a deterministic neighbor, and closes only after durable absence is observable. | Delivered bounded slice | Archive, undo/recovery/export, project-owned external assets, deleted-route recovery, and broader device/accessibility evidence. |
...
| Local project catalog | Delivered baseline | P0 | F0/F2 | Schema-2 transactional JSON loads, explicitly migrates v1, creates/selects/renames/duplicates/hard-deletes, and persists catalog mutations. |
...
| Project lifecycle | Foundation | P0 | F2 | Rename, transactional duplication, and bounded transactional hard deletion are delivered; add archive, recovery/undo/export, project-owned asset policy, lineage/provenance policy, and broader consistency checks. |
