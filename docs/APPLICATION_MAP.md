...
| Adaptive UI | Delivered | Bottom navigation is used on compact widths and a navigation rail at 720 dp or wider. Rail labels/header collapse to five accessible icons below 520 dp height. Dashboard metrics, Studies fields/results, RF and project-name editor fields, the map canvas/site list, and Data cards use bounded responsive layouts. The Activity resizes for the IME so the short landscape project-name editor retains its app bar and editable value. | The shell and bounded feature density are adaptive; wider device coverage and full adaptive list/detail patterns still need testing. |
...
| Project operations | Foundation | Load, create, select, rename, and transactional mutation are delivered; rename preserves project identity and its RF graph and rejects a stale competing name, while Add RF Path persists one linked network/site/sector/receiver without exposing partial state. | Delete, duplicate, archive, import, independent RF-entity CRUD, and impact-aware linked deletion are not delivered. |
...
| Projects and nested editors | Foundation | Projects lists/selects/creates projects, opens a saveable rename editor that transactionally changes only the selected project name, and opens a saveable Add RF Path editor that validates and transactionally adds one selected-system RF network, transmitter site/sector, and receiver. | Rename is one bounded update slice and Add RF Path is one combined create slice, not complete project lifecycle or independent RF-entity CRUD; broader device testing remains. |
...
| Instrumented tests | Delivered baseline | The current 20-test suite passes on an Android 16/API 36 emulator, exercising supported, unknown, malformed, nested RF-path, and nested project-name route restoration; explicit mutation-completion and transient pending-save recovery; legacy-name normalization; competing-rename and draft-discard protection; accessibility semantics; persisted project rename; and create-project -> persist-RF-path -> Activity-recreation behavior. The preceding 18-test revision passed on the physical Android 16 reference phone. | A fresh physical run of the current suite, true process termination, broader accessibility automation, a formal device matrix, and CI execution remain. |
...
| **Projects** | Compact wrapping catalog cards, selection/create dialog, schema/details, and entry to nested project-name and Add RF Path editors. | Foundation | Remaining project lifecycle operations, independent network/site/sector/receiver CRUD, and broader device/accessibility evidence. |
| **Rename Project** | Saveable compact name draft, explicit impact statement, transactional local save, durable-success return, normalized dirty-exit protection, and stale competing-rename rejection. | Delivered bounded slice | Metadata editing, duplicate/archive/delete, richer conflict diagnostics, and broader device/accessibility evidence. |
...
| Local project catalog | Delivered baseline | P0 | F0/F2 | Schema-2 transactional JSON loads, explicitly migrates v1, creates/selects/renames, and persists catalog mutations. |
...
| Project lifecycle | Foundation | P0 | F2 | Rename is delivered; add duplicate, archive, delete, recovery, and consistency checks. |
