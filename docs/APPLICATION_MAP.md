...
11. run JVM and Android tests for the delivered domain, persistence, project-link study, dataset, UDF, form, and navigation behavior.

This slice is not yet the mobile engineering MVP defined later in the roadmap. The persisted project-link result is a deliberately bounded free-space record: there is no cartographic basemap, DEM or terrain profile, Earth-curvature clearance, LOS or Fresnel-clearance analysis, diffraction, clutter, directional antenna-pattern loss, raster coverage, study export, desktop-project interchange, `.rp3` support, or full RadioPlanner parity.
...
| Artifact store | Foundation | A private content-addressed store can stage, size-bound, SHA-256-verify, deduplicate, inspect, and copy immutable artifact bytes. Project schema 5 can persist bounded artifact references and structural records that may point to them. | No user import workflow, artifact-management UI, project attachment flow, garbage collection, export package, ownership cleanup, or regional dataset acquisition is delivered. The project-link record is persisted inside its project document and does not make the artifact store a completed result-package workflow. The separate release-managed IBGE reference package does not make an antenna, GIS, coverage, regulatory, or report workflow functional. |
...
| JVM tests | Delivered baseline | The current 252-test JVM suite passes with no failures or skips. In addition to the prior model, RF, geographic, persistence, project-operation, dataset, form, ViewModel, and English-only coverage, it verifies mean-Earth endpoint geometry including antimeridian behavior, AGL-only inclined distance, compatibility profiles, canonical fingerprint and strict JSON round trip, result invariants and tolerances, stale/missing/incompatible/id-collision outcomes, persist-before-publish behavior, storage-failure rollback, duplication history, schema-4-to-5 migration ordering, and preservation of the authoritative schema-4 index when migration publication fails. | Independent desktop/RadioPlanner parity fixtures, terrain/LOS/Fresnel-clearance goldens, property testing, accessibility, performance, export, transformer fault automation, and complete system-flow coverage remain. |
| Instrumented tests | Delivered baseline | The complete connected suite contains 68 tests with no failures or skips on the Android 16/API 36 `Medium_Phone_API_36.1` emulator at system font scale 1.30. All 5 `StudiesScreenTest` cases cover 360 × 480 dp/font-scale-1.30 selector/action/limitation reachability, complete saved terms/fingerprint/`NoData` warnings, lazy timestamp-ordered history, collision-safe sector identity, and an explicit no-compatible-receiver state. A real-storage `FileProjectRepositoryMigrationTest` covers indexed project-schema-4-to-5 promotion, integrity, injected-field stripping, absence of `AtomicFile` sidecar residue, retention of the immutable legacy document, and stable reopen on the same emulator. The preceding 18-test revision passed on the physical Android 16 reference phone. | A fresh physical run, API 23 dataset execution, true system-reclaim process termination, broader accessibility automation, a formal device matrix, and CI execution remain. |
...
```mermaid
flowchart TD
    CURRENT[Current Compose/typed Nav3/store-schema-1 index/project-schema-5 documents/project lifecycle/independent RF CRUD] --> HARDEN[System recovery, accessibility, jobs, and ownership decisions]
