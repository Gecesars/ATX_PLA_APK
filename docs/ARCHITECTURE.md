...
| Domain | Foundation with bounded regulatory/coverage slice | Schema-6 project/RF/study models and validation are implemented, including source/canonical antenna artifacts, fixed one-degree HRP/VRP, gain-bound normalized-content identity V2, warnings, and sector assignments; the link-study record remains the schema-5 historical addition. Separate transient models cover terrain-backed digital-TV contour evidence, protected-boundary D/U points, bounded operational field surfaces, and non-regulatory legacy E(50,10) reference envelopes. Anatel, IBGE, regional inventory/jobs, reconciliation, runner outcomes, and WorkManager identities remain explicit. | Anatel rows are read-only reference inputs, not project transmitter records or authorization proof. Antenna records are separable cuts, not full-wave 3D truth. Current-regulatory interfering iso-field claims, strict FM interference, RF clutter, interpreted building heights, persisted executable coverage scenarios, and filing conclusions remain absent. |
...
| Tests | Delivered evidence | The current JVM aggregate discovered 602 tests: 601 passed, one Windows symlink-hardening case was permission-skipped, and there were zero failures or errors. The Android 16/API 36 AVD aggregate passed 99/99 tests with no failures or skips, including arbitrary-element composer controls and enlarged-text coverage render-mode/NoData reachability. A separate live official Anatel run indexed 87,400 records and exercised offline FM/TV queries, source details, and replacing pagination. | Evidence is not strict cross-engine contour parity, a production durable-job caller, API 34+ UIDT, late network-byte/checkpoint crash recovery, API 23/33 runtime, or process/reboot recovery. A fresh physical run, broader accessibility automation, map/performance evidence, true system-reclaim process death, and a formal device matrix remain. |
...
The remote basemap is a visual presentation layer rather than an RF input, authorized offline package, or general GIS result. The service-contour layer only draws supplied transient local results and does not recalculate or persist them. Neither the delivered IBGE attribute package nor cached regional DSM/land-cover/building outputs are consumed by this screen or the contour planner. The IBGE package contains no renderable polygons; the regional TIFF output is metadata-only, and the experimental building GeoJSON has no map or RF integration. G4 still requires an authorized local package lifecycle, general hostile-package handling, bare-earth DTM/raster sampling, general geometry integration, airplane-mode package proof, provider eligibility approval, and broader performance evidence. Pixel-level raster `NoData` is not present; contour `NoData` is a separate explicit result state. See `docs/BASEMAPS.md` for exact provider and cache boundaries.
...
The current JVM aggregate discovered 602 tests: 601 passed, one Windows symlink-hardening case was permission-skipped, and there were zero failures or errors across persistence, regional-data/job, WorkManager-contract, antenna composition/interchange/identity, Anatel/IBGE, RF, coverage, strict DTV, and export paths. The Android 16/API 36 AVD aggregate passed 99/99 tests with no failures or skips across navigation, compact UI, arbitrary antenna-composer controls, coverage mode/NoData accessibility, repositories, persistence, map, and scheduler foundations. `lintDebug` completed with 0 errors, 19 warnings, and 3 informational findings; debug APK and Android-test APK assembly succeeded. A separate live official Anatel run indexed 87,400 records and exercised offline FM/TV queries, source details, and replacing 25-record pagination. This is bounded API 36 reference evidence, not strict cross-engine contour parity, API 23/33 foreground-runtime, UIDT, late network-byte/checkpoint crash, physical-device, or process/reboot evidence.
