...
| Domain | Foundation with bounded regulatory/coverage slice | Schema-6 project/RF/study models and validation are implemented, including source/canonical antenna artifacts, fixed one-degree HRP/VRP, gain-bound normalized-content identity V2, warnings, and sector assignments; the link-study record remains the schema-5 historical addition. Separate transient models now cover terrain-backed digital-TV contour evidence, protected-boundary D/U points, and a bounded operational field surface. Anatel, IBGE, regional inventory/jobs, reconciliation, runner outcomes, and WorkManager identities remain explicit. | Anatel rows are read-only reference inputs, not project transmitter records or authorization proof. Antenna records are separable cuts, not full-wave 3D truth. Regulatory interfering iso-field contours, RF clutter, interpreted building heights, persisted executable coverage scenarios, and filing conclusions remain absent. |
