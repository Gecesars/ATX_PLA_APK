...
> Architecture baseline and target as of September 1, 2026. The repository contains a working Compose application, typed/saveable Navigation 3 routes, explicit UDF and use-case boundaries, a transactional schema-6 project store, a content-addressed artifact store, validated RF-domain models, transactional project/RF operations, a CPU-only Antenna Pattern Lab, a bounded RF calculator, a persisted project-linked ITU-R P.525-5 study, pattern-aware non-regulatory broadcast overlays, and a bounded current-rule FM/digital-TV regulatory workflow. That workflow uses on-demand SHA-256-evidenced ANADEM DTM ranges, verified IBGE 2022 urban geometry, a verified MCom FM/GTVD/RTVD baseline, complete bounded candidate execution, bidirectional P.526-15 Deygout-Assis D/U, urban-area gates, and existing-versus-proposed classifications with map and HTML/PDF/XLSX evidence. Analog TV/RTV is excluded. The review-gated Anatel Basic Plan workflow, verified offline IBGE attributes, screen-bound regional raw-data flow, and durable regional-job foundations remain separate subsystems. Sections labeled Target or Planned are not delivered. The regulatory workflow is not filing-ready without independent numerical parity and qualified professional/legal/source-license review; the general regional cache remains DSM/categorical data rather than a general terrain/clutter engine; antenna cuts are not full-wave or measured 3D results; standalone contours remain non-regulatory; and complete RadioPlanner parity is not claimed.
...
flowchart TD
    ...
    BCP --> MAP
    BVM --> BPLAN[Bounded visible XYZ tile planner]
    BVM --> BCACHE[(Validated private basemap tile cache)]
    BPLAN --> MAP
    BCACHE --> MAP
...
| Domain | Foundation with bounded regulatory/coverage slice | Schema-6 project/RF/study models and validation are implemented, including source/canonical antenna artifacts, fixed one-degree HRP/VRP, gain-bound normalized-content identity V2, warnings, and sector assignments; the link-study record remains the schema-5 historical addition. Separate transient models cover terrain-backed FM/digital-TV protected contours, bidirectional protected-boundary D/U evidence, class fallbacks, bounded operational field surfaces, and non-regulatory legacy E(50,10) reference envelopes. Anatel, IBGE, regional inventory/jobs, reconciliation, runner outcomes, and WorkManager identities remain explicit. | Anatel rows are read-only reference inputs, not project transmitter records or authorization proof. Antenna records are separable cuts, not full-wave 3D truth. Current-regulatory interfering iso-field claims, mixed-service rules, RF clutter, interpreted building heights, persisted executable coverage scenarios, and filing conclusions remain absent. |
...
| Brazil broadcast contours and D/U | Delivered FM/digital-TV same-service engineering workflow | `BrazilBroadcastContourPlanner` retains source-pinned standalone non-regulatory overlays. `BrazilBroadcastRegulatoryStudyPlanner` keeps the project transmitter independent and consumes on-demand ANADEM v1.0 DTM ranges, official IBGE 2022 geometry, a current verified MCom FM/GTVD/RTVD baseline, and optional read-only Basic Plan references. It delivers 72-radial P.1546 protected contours, bidirectional P.526-15 Deygout-Assis D/U, the 50%/70% whole-municipality urban gate, adjacent-digital colocation, and existing-versus-proposed classifications without a nearest-reference cap. The map and HTML/PDF/XLSX exporters preserve method, direction, margin, source-integrity scope, `NoData`, and blockers. | Analog TV/RTV and former TV channels 5/6 are excluded. Independent normative vectors, durable executable state, receiver-pattern/other-service rules, professional/legal/source-license review, and current regulatory iso-field products remain blocked; no filing conclusion is claimed. |
| Engineering map | Delivered bounded slice | Compose renders a Web Mercator coordinate grid using pure domain camera math, project fit, pan, anchored pinch zoom, metric scale, sites, azimuths, contours, and a stale-safe site move. Six fixed remote XYZ providers can render validated visible tiles from a private bounded cache below local engineering geometry; attribution and provider terms remain visible. Protected, statistical-screening, incomplete, and `NoData` contour states remain distinct. SAF exports supplied overlays as deterministic verified KMZ. | No authorized offline basemap package/Natural Earth fallback, DEM sampling, terrain, IBGE polygon/map integration, general GIS features, receiver move, complete provider eligibility approval, or broad map performance/device gate is delivered. The basemap is visual only and the contour path/KMZ remains a dedicated transient non-regulatory reference. |
| Anatel Basic Plan catalog | Delivered bounded slice | After an explicit UI review acknowledgement, `AndroidAnatelBasicPlanCatalog` downloads the complete pinned `Canais.zip` over allowlisted HTTPS, streams and syncs a bounded immutable SHA-256-named raw artifact plus provenance, parses its exact XML entries into a transactionally staged/synced immutable SQLite schema-v1 index, and atomically switches the current pointer only after validation. The core queries service plus optional state, municipality/code, channel, frequency range, accent-insensitive text, or Basic Plan ID with bounded paging; the compact UI exposes service/state/channel/text and replacing 25-record previous/next pages. The broadcast runner consumes installed nationwide cochannel/first-adjacent partitions as transient read-only references. | No archive is bundled. License remains `REVIEW_REQUIRED`; review acknowledgement is not redistribution approval. Refresh is route-scoped and has no byte progress, background/process/reboot survival, conditional GET, current catalog-metadata/license resolution, automatic eviction after the raw eight-archive/512 MiB or index eight-file/768 MiB ceilings, or durable project/snapshot pinning and diff. |
| Dataset catalog | Delivered bounded slices | The screen prepares and queries the release-managed national IBGE package, exposes the review-gated Anatel offline catalog, and plans/acquires a small regional envelope from fixed Copernicus GLO-30 DSM, WorldCover, and experimental OSM `building`/`building:part` way sources. The digital-TV runner can reopen verified Copernicus elevation artifacts through its terrain mosaic for the transient study. Separate regional job-contract/store/reconciliation/shared-runner and API 23-33 WorkManager foundations exist outside the screen path. | Anatel and regional network work retain different persistence contracts. No process-durable Anatel refresh exists; no regional reconciliation executor submits the delivered adapter, and API 34+ UIDT remains absent. Bare-earth DTM approval, clutter interpretation, general geometry/map integration, exact containment, and population-by-coverage remain missing. |
| Tests | Delivered physical evidence | The current JVM aggregate discovered 616 tests: 615 passed, one Windows symlink-hardening case was permission-skipped, and there were zero failures or errors. The physical Android 16/API 36 aggregate passed 100/100 tests with no failures or skips on model `25080RABDG`, including arbitrary-element composer controls, the independent FM current-rule card, and enlarged-text coverage render-mode/`NoData` reachability. A separate dated official Anatel run indexed 87,400 records and exercised offline FM/TV queries, source details, and replacing pagination. | Evidence is not strict cross-engine contour parity, a production durable-job caller, API 34+ UIDT, late network-byte/checkpoint crash recovery, API 23/33 runtime, or process/reboot recovery. Broader accessibility automation, map/performance evidence, true system-reclaim process death, and a formal device matrix remain. |
| Build automation | Delivered baseline | CI runs unit tests, lint, and debug/test APK assembly. The current local aggregate completed `lintDebug` with 0 errors, 19 warnings, and 3 informational findings and successfully assembled the debug APK and compiled Android tests. | Connected test remains outside current CI; signed release automation is absent. |
...
- `BrazilBroadcastRules` pins Anatel Acts 8104/2022 and 9751/2022 as checked on September 1, 2026;
- FM protected reference uses `E(50,50)` at 66 dBµV/m;
- digital TV uses `E(50,90) = 2 × E(50,50) − E(50,10)`, at 43 dBµV/m for channels 7–13 and 51 dBµV/m for channels 14–51;
...
- current Anatel point-to-point P.526 plus Assis and D/U evaluation inside this bounded P.525 project-link engine; the separate broadcast workflow supplies its own same-service boundary study;
...
### Current geographic viewport and interactive basemap

The current `EngineeringMapScreen` is a bounded geographic implementation:
...
- renders a coordinate grid, metric scale, camera center, supplied broadcast-contour references, site markers, and active-sector azimuth rays above an optional raster basemap;
- exposes six fixed HTTPS XYZ providers matching the desktop remote catalog within a hard 10-provider ceiling, or a grid-only mode;
- plans only the human-visible viewport at one density-aware raster zoom, caps each plan at 48 unique tiles, and has no region/multi-zoom/background-prefetch API;
- validates bounded PNG/JPEG/WebP payloads before atomic promotion into a private 128 MiB cache, retaining valid stale tiles when refresh fails;
- keeps selected-provider attribution visible on the map and exposes usage notice, terms link, tile status, and cache size in a compact control card;
...
- provides a semantic content description that identifies either the selected basemap or the offline coordinate-grid fallback.

The remote basemap is a visual presentation layer rather than an RF input, authorized offline package, or general GIS result. Standalone service-contour overlays remain transient and do not recalculate or persist themselves. The separate regulatory-study path now consumes on-demand ANADEM DTM ranges and official IBGE municipality geometry, and its map can draw the attached urban-sector rings at close zoom; the bundled national IBGE attribute package itself still contains no renderable polygons. The general regional TIFF path remains metadata/processed-grid infrastructure, while WorldCover and experimental building GeoJSON are not regulatory RF loss inputs. G4 still requires an authorized local package lifecycle, general hostile-package handling, general geometry integration, airplane-mode package proof, provider eligibility approval, and broader performance evidence. `NoData` remains explicit in every regulatory raster/profile result. See `docs/BASEMAPS.md` for exact provider and cache boundaries.
...
The current JVM aggregate discovered 616 tests: 615 passed, one Windows symlink-hardening case was permission-skipped, and there were zero failures or errors across persistence, regional-data/job, WorkManager-contract, antenna composition/interchange/identity, Anatel/IBGE, RF, coverage, current FM/DTV protected-contour and bidirectional D/U, and export paths. The physical Android 16/API 36 aggregate passed 100/100 tests with no failures or skips across navigation, compact UI, the independent FM current-rule card, arbitrary antenna-composer controls, coverage mode/`NoData` accessibility, repositories, persistence, map, and scheduler foundations. `lintDebug` completed with 0 errors, 19 warnings, and 3 hints; debug APK and Android-test APK assembly succeeded. A separate dated official Anatel run indexed 87,400 records and exercised offline FM/TV queries, source details, and replacing 25-record pagination. This is bounded API 36 reference evidence, not strict cross-engine contour parity, API 23/33 foreground-runtime, UIDT, late network-byte/checkpoint crash, process/reboot evidence, or a formal device matrix.
...
11. validate the delivered bounded FM/digital-TV study against independent P.1546/P.526-Assis goldens, add mixed-service rules and durable executable-state provenance, and obtain professional/legal/source-license review; keep analog TV/RTV excluded, standalone overlays `regulatory = false`, and filing readiness fail-closed until every gate closes;
...
## Transient digital-TV study and coverage pipeline

The current CPU-only digital-TV slice deliberately remains outside the durable project document:

```text
independent project TV sector
        +
verified regional terrain mosaic
        +
read-only Anatel Basic Plan references
        |
        v
72 radial HNMT + P.1546 E(50,90) protected contour
        |
        +--> 181 x 181 operational field surface
        |      + verified HRP/VRP, or disclosed omnidirectional fallback
        |      + broadcast / continuous / Turbo rendering
        |
        +--> protected-boundary P.526-15 Deygout-Assis D/U points
        |
        +--> HTML / PDF / XLSX / KMZ export with read-back SHA-256 verification
```

The Compose map receives already calculated immutable values for the active process lifetime. It
does not call the propagation engine during drawing. Layer order is basemap, coverage surface,
coordinate grid, vector contours, D/U evidence, then sites. `NoData` cells are transparent and do
not participate in classification. The operational surface is not the protected regulatory
contour, and protected-boundary D/U dots are not an interfering iso-field contour.

The result remains transient because schema, artifact ownership, stale-input behavior, and recovery
for executable regulatory studies have not yet passed their persistence gate. Exported artifacts
carry evidence, but importing them does not recreate an executable study. See
`ANTENNA_COVERAGE_PARITY.md` for the detailed capability boundary.
