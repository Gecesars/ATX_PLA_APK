...
> Evidence baseline: September 1, 2026. The repository now contains an adaptive Compose shell, typed and saveable Navigation 3 routes, explicit UDF/application boundaries, a schema-6 project model stored through a small atomic index and immutable SHA-256 project documents, explicit legacy migration through schema 6 while preserving schema 5 as the link-study milestone, transactional project lifecycle operations, independent network/site/sector/receiver CRUD, a compact Manage RF Assets UI, a content-addressed artifact store with a bounded antenna workflow, a CPU-only Antenna Pattern Lab with strict codecs and coherent array composition, a bounded manual RF calculator, a persisted project-linked ITU-R P.525-5 study, pattern-aware non-regulatory broadcast overlays, and a bounded current-rule FM/digital-TV workflow. The regulatory workflow now consumes on-demand ANADEM bare-earth DTM ranges, official IBGE 2022 urban geometry, and a verified MCom licensed baseline for uncapped bounded bidirectional D/U, urban-area coverage, and existing-versus-proposed classification with HTML/PDF/XLSX evidence. Analog TV/RTV is excluded, while extended FM uses channels 141–197. The repository also includes the review-gated Anatel Basic Plan catalog, verified offline IBGE national attributes, regional job foundations, and physical Android instrumentation. Filing readiness remains blocked by independent numerical validation and qualified professional/legal/source-license review; no Anatel authorization conclusion, process-durable regulatory run, full-wave antenna solution, or complete desktop/RadioPlanner parity is claimed.
...
7. require physical-device evidence for current product milestones; emulator evidence is historical only;
...
| Build and lint | Delivered | Debug/test APKs built; latest lint has 0 errors, 19 warnings, and 3 hints. | Dependency/toolchain update advisories and no signed release. |
...
| Compact phone information density | Delivered baseline | Scalable compact typography and shared components, 12–16 dp feature gutters, responsive metrics/fields/cards/map height, a short-height rail, IME resizing for the project-name editor, and explicit 48 dp minimums have manual evidence on one physical Android 16 phone: 1280 × 2772 pixels, density 520, portrait at font scales 1.15/1.30 and landscape at 1.15. The 100-test physical aggregate adds 360 × 480 dp/font-scale-1.30 reachability for Manage RF Assets, Data Catalog, Engineering Map, Antenna Pattern Lab, project studies, and an independent FM current-rule study. | This single physical-device observation is not a complete OEM/API/aspect-ratio/theme/font-scale/accessibility matrix; the app does not clamp or override system font scale. |
...
| Engineering canvas | Delivered bounded slice | A Compose Web Mercator viewport plots fixed remote XYZ basemaps, an operational `181 x 181` field surface, coordinate grid, protected contours, red dash-dot legacy E(50,10) envelopes, current protected-boundary D/U evidence, reference stations, sites, and active azimuths in a deterministic order. Coverage offers broadcast-band, continuous, and Turbo heatmap modes with explicit units, range, palette identity, and transparent `NoData`. Visible tile plans remain capped at 48, provider count at 10, validated files at 2 MiB each, and private cache at 128 MiB. | The remote basemap is visual only and not an authorized offline package or RF/GIS input. Natural Earth/local package lifecycle, provider distribution eligibility, IBGE geometry, general GIS, current-regulatory iso-field claims, receiver editing, broader external-reader validation, and map performance/device evidence remain. |
| Data Catalog | Delivered bounded slices | The bundled 21.1 MiB IBGE asset installs to a verified 67.6 MiB read-only SQLite database. The Anatel section review-gates an explicit whole-archive HTTPS refresh, shows verified immutable snapshot evidence, and queries its offline SQLite v1 generation by service/state/channel/text. The broadcast runner consumes nationwide cochannel/first-adjacent partitions from the installed snapshot as transient read-only references. A separate regional flow plans and acquires fixed Copernicus GLO-30 DSM and WorldCover COGs plus tiny opt-in OSM `building`/`building:part` way responses using bounded transport, provenance, inventory, processing, and cache rules. Regional durable-job foundations add canonical E6 plans, dual fingerprints, strict state/CAS, private records, pure reconciliation, a shared runner, and an API 23-33 foreground WorkManager adapter. | Anatel refresh lacks byte progress, background/process survival, conditional metadata refresh, retention cleanup, durable project/snapshot pinning and diff, and approved license status. Regional user execution is also screen-bound and no production reconciliation executor invokes its scheduler. Broader raster, terrain, clutter, geometry, map, coverage, process/reboot, API 34+ UIDT, and release-license work remains. |
| Anatel Basic Plan catalog | Delivered bounded slice | Official source/license/provenance/frequency models, bounded hostile ZIP/XML parsing, a review-gated production HTTPS refresh, immutable SHA-256 raw/provenance snapshots, transactionally staged and atomically promoted SQLite schema-v1 indexes, an atomic current pointer with prior-generation preservation, explicit domain `NoData`, bounded core queries, a compact offline service/state/channel/text UI with `CHECKING`/`NOT_ACQUIRED`/`READY`/`REFRESHING`/`FAILED` phases and replacing 25-record previous/next pages, and transient read-only broadcast-study integration are implemented. | No official archive is bundled. The license remains `REVIEW_REQUIRED`; refresh ownership is tied to the Data route, but the blocking catalog call is not cooperatively cancelable. There is no current metadata/license resolver, conditional GET, byte progress, WorkManager/UIDT, process/reboot survival, automatic cleanup after the raw eight-archive/512 MiB or index eight-file/768 MiB ceilings, or durable project/snapshot pinning and diff. |
...
| Brazil broadcast regulatory study | Delivered FM/digital-TV same-service engineering workflow | The project transmitter remains independent. On-demand ANADEM DTM ranges, official IBGE 2022 urban polygons, and a verified current MCom FM/GTVD/RTVD snapshot now feed uncapped bounded candidate execution, FM `E(50,50)` or digital-TV `E(50,90)` protected contours, bidirectional P.526-15 Deygout-Assis D/U, the 50%/70% whole-municipality urban gate, adjacent-digital colocation, operational coverage, and licensed existing-versus-proposed status. Basic Plan data remains read-only. HTML/PDF/XLSX evidence includes source-integrity scope and `NoData`. | Analog TV/RTV and former TV channels 5/6 are excluded. Filing still requires independent numerical validation and qualified professional/legal/source-license review. Other-service rules, receiver-pattern exceptions, current-regulatory iso-fields, durable executable state, and complete desktop parity remain open. |
| JVM tests | Delivered evidence | The current aggregate discovered 616 tests: 615 passed, one Windows symlink-hardening case was permission-skipped, and there were zero failures or errors. `lintDebug` completed with 0 errors, 19 warnings, and 3 hints; debug APK and Android-test APK assembly succeeded. | The tests do not prove a production durable-job caller, late network-byte/checkpoint crash recovery, API 23/33 runtime behavior, process/reboot recovery, current-regulatory iso-interfering contour parity, or complete desktop parity. |
| Instrumented tests | Delivered physical evidence | The current physical Android 16/API 36 aggregate passed 100/100 tests with no failures or skips on model `25080RABDG`, including arbitrary-element composer interaction, current FM study reachability at 360 × 480 dp/font scale 1.30, legacy-contour map semantics, enlarged-text coverage render-mode/`NoData`, Anatel/IBGE repositories, persistence, and WorkManager foundations. A separate dated official Anatel run downloaded and indexed 87,400 source records and exercised offline FM/TV query, source details, and page replacement. | API 23 and API 33 scheduler execution, true system-reclaim process termination, reboot recovery, rotation/device matrix, broader accessibility automation, map performance, and CI execution remain. |
...
- **Delivered bounded slice:** release-managed national IBGE sector attributes and municipality summaries with offline verification/search; the regulatory study additionally downloads a verified official UF GeoPackage on demand and reports an area-weighted population estimate, while a general offline national geometry package is not delivered;
...
- durable arbitrary antenna-array design persistence and independently validated full-wave comparison beyond the delivered bounded CPU composer/editor;
- complex GIS editing;
...
| **G4 — Map and data** | Renderer, offline format, attribution, license, NoData, disk budget, and lifecycle approved. | **Foundation:** the tested Compose renderer now includes a bounded six-provider interactive XYZ layer, validated 128 MiB visible-tile cache, visible attribution, and grid fallback. The bounded national IBGE package and regional acquisition foundations are also delivered. G4 remains open for an authorized offline package, provider distribution approval, general package lifecycle, bare-earth DTM/raster sampling, GIS integration, ownership/removal, broader performance/device evidence, and IBGE redistribution approval. | Offline field-map, terrain, polygon, and population-by-coverage claims. |
...
- compact responsive feature information density with bounded physical Android 16 portrait checks at approximately 394 dp and font scales 1.15/1.30 plus a baseline landscape check; the physical 100-test aggregate includes compact 360 × 480 dp/font-scale-1.30 UI coverage;
...
- CI workflow plus a current local aggregate with 0 lint errors, 19 warnings, 3 hints, and successful debug APK/Android-test APK assembly;
- current green 604-test JVM aggregate with one permission-skipped Windows symlink case, plus a 100/100 physical Android 16/API 36 aggregate and dated live Anatel archive/index/query evidence;
...
**State:** Geographic viewport, bounded six-provider interactive XYZ basemap, durable site move, bounded offline IBGE attributes, the review-gated Anatel Basic Plan catalog, fixed-source regional raw-data acquisition/processing, and regional job contract/store/reconciler/shared-runner foundations are delivered. The basemap fetches only current visible tiles into a private validated cache and is not an offline package. The Anatel path performs one foreground whole-archive HTTPS refresh and atomically publishes an offline SQLite v1 generation; it has no background/process survival or automatic retention management. An API 23-33 WorkManager compatibility envelope can call the regional runner, but the production screen and reconciliation path do not submit or observe it. An authorized offline basemap, bare-earth DTM/raster sampling, append-only snapshot ownership, general packages, API 34+ UIDT, user-reachable scheduled/process-durable regional execution, late network-byte/checkpoint crash evidence, and sector geometry remain planned.
...
- a fixed six-provider XYZ catalog with visible-view-only planning, raster validation, private bounded caching, provider attribution/terms, and an offline grid-only fallback;
...
These sub-slices do not complete the phase vertical slice because the interactive remote basemap is not an authorized offline package, and the map does not sample a DEM, consume sector geometry, or calculate population by coverage.
...
**State:** In progress. The digital-TV workflow now delivers one bounded, cancelable `181 x 181` operational field surface with P.1546 field values, terrain/HNMT input, verified HRP/VRP shaping, explicit `NoData`, and three map render modes. It does not close F5 because scenarios/results remain transient and best server, overlap, C/(I+N), comparison, general models, and GeoTIFF are absent.
...
- general tiled/cancelable small grids beyond the delivered fixed digital-TV surface;
...
**Delivered sub-slice:** run the independent digital-TV study → calculate terrain/HNMT radials → produce a bounded field surface → render broadcast bands, continuous color, or heatmap above the basemap and below the vector evidence → disclose units, palette, range, pattern fallback, and `NoData`.
...
| MOB-033 | Add Brazil broadcast-contour reference plotting and current-rule boundary study | Delivered FM/digital-TV same-service engineering workflow | P2 | MOB-012/013 | CPU-only FM `E(50,50)` and digital-TV `E(50,90)` protected profiles, revoked/non-regulatory E(50,10) references, 72 radial pattern-shaped ERP, on-demand ANADEM terrain, official IBGE urban coverage, nationwide Basic Plan references, a current MCom licensed baseline, uncapped bounded bidirectional P.526-15 Deygout-Assis D/U, colocation, existing-versus-proposed classification, compact census map geometry, and HTML/PDF/XLSX evidence are delivered. Analog TV/RTV and TV 5/6 are excluded. Independent parity, persisted executable studies, professional review, and filing approval remain open. |
...
| Anatel Basic Plan catalog | Current domain/ViewModel tests cover bounded query validation, domain `NoData` to UI-phase mapping, the source-review gate, dirty filters, replacing 25-record previous/next pages, and nationwide same-service channel-partition consumption by the broadcast runner. Instrumented repository tests exercise a synthetic complete refresh through immutable raw/provenance storage and SQLite publication, offline reducing queries, and failed staged-parse preservation of both the prior current index and the newly verified raw evidence; compact UI instrumentation exercises the review gate and visible states. Live official-source behavior, metadata/license resolution, conditional requests, retention-full recovery, process/background survival, and durable project/snapshot pins/diffs remain outside that evidence. |
...
| Broadcast contour and D/U study | Profile/band thresholds, FM `E(50,50)` and DTV `E(50,90)` transforms, packaged table identity, peak/pattern-shaped radial ERP, periodic interpolation/no-double-gain fixtures, on-demand ANADEM DTM, official urban-sector geometry, complete bounded candidate execution, licensed-baseline comparison, bidirectional P.526-15 Deygout-Assis point evidence, adjacent-DTV ERP comparison, explicit `NoData`, deterministic fingerprints, compact rendering, and bounded HTML/PDF/XLSX/KMZ evidence export. Analog TV/RTV is excluded. Independent official goldens, mixed-service rules, persisted executable state, external-reader evidence, broader device-performance evidence, professional review, and regulatory approval remain exit evidence. |
...
| A standalone P.1546 overlay or bounded current-rule D/U study is mistaken for a filing-ready conclusion | Invalid filing, coordination, or service decision | Keep standalone overlays `regulatory = false`; keep current-study blockers fail-closed; expose terrain type, pattern/fallback, table/ruleset identity, statistical basis, candidate cap, bidirectional P.526/Assis evidence, margin, `NoData`, immutable export provenance, and the absent urban-coverage/licensed-baseline/independent-validation/professional-review gates. |
...
| An installed Basic Plan catalog is mistaken for authorization evidence | Stale, unlicensed, or unauditable channel decisions | Keep source review, verified raw acquisition, indexed generations, project pins, snapshot comparison, and licensing conclusions as separate gates. The broadcast study consumes rows only as transient read-only references and never copies them into the project transmitter; the catalog license remains `REVIEW_REQUIRED`, current official metadata is unresolved, and the exact snapshot is not durably pinned to a project or executable study. |
