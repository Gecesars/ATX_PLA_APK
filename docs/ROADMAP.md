...
| Brazil broadcast and digital-TV study | Delivered bounded slices | The planning-reference overlay retains its 72-radial FM/DTV profiles, HRP shaping, explicit fallback, and non-regulatory state. A separate CPU-only DTV workflow keeps the project transmitter independent, samples acquired terrain for HNMT, derives a P.1546 `E(50,90)` protected contour, evaluates P.526-15 Deygout-Assis D/U at every protected-boundary radial, produces a terrain-aware operational coverage surface with verified HRP/VRP shaping, and exports HTML/PDF/XLSX/KMZ evidence. | Filing readiness still requires a bare-earth DTM, current verified Basic Plan snapshot, complete contour/D/U gates, independent numerical validation, and legal review. Regulatory interfering iso-field contours, strict FM interference, durable executable study state, and complete desktop parity remain open. |
| JVM tests | Delivered evidence | The current aggregate discovered 598 tests: 597 passed, one Windows symlink-hardening case was permission-skipped, and there were zero failures or errors across project, regional-data/job, WorkManager-contract, antenna, Anatel/IBGE, RF, coverage, strict DTV, and export paths. `lintDebug` completed with 0 errors, 19 warnings, and 3 hints; debug APK and Android-test APK assembly succeeded. | The tests do not prove a production durable-job caller, late network-byte/checkpoint crash recovery, API 23/33 runtime behavior, process/reboot recovery, regulatory iso-interfering contour parity, or complete desktop parity. |
| Instrumented tests | Delivered evidence | The current Android 16/API 36 AVD aggregate passed 98/98 tests with no failures or skips, including enlarged-text coverage render-mode/NoData reachability and multipanel/cosine antenna-composer interaction. A separate live official Anatel run downloaded and indexed 87,400 source records and exercised offline FM/TV query, source details, and page replacement. The preceding 18-test physical-phone revision remains historical evidence. | A fresh physical run, API 23 and API 33 scheduler execution, true system-reclaim process termination, reboot recovery, rotation/device matrix, broader accessibility automation, map performance, and CI execution remain. |
