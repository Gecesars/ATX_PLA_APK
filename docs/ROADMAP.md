...
| UDF/ViewModel | Foundation | Explicit `AppUiAction`/`AppUiEffect`, structured problem/recovery values, injected use cases/dispatchers, cancellation-aware calculation, serialized catalog mutations, and the non-executing regional job contract/store/reconciler exist. The Data Catalog has a separate feature ViewModel with preparation and debounced query states. Every project-catalog mutation is evaluated against the latest durable catalog inside the repository transaction; state is published only after persistence, while rejected/no-op outcomes rebase to the latest catalog without writing. | Additional feature-level ViewModels, cross-instance catalog observation, the DI/scoping decision, regional job runner/scheduler/UI wiring, broader observability, accessibility, and system recovery evidence remain. |
...
| Artifact store | Foundation | A private content-addressed store stages, operation-bounds, SHA-256-verifies, deduplicates, checks, and copies immutable bytes. Schema 5 can persist bounded artifact references. The regional raw-data foundation uses a separate private cache and inventory. | There is no import or attachment workflow, artifact-management UI, garbage collection, export package, project-deletion cleanup, or reference-aware regional removal. The persisted project-link record lives in its project document; it is not an artifact-backed portable package. Neither the IBGE package nor the regional cache delivers the engineering workflows represented by artifact roles. |
...
| Data Catalog | Delivered bounded slices | The bundled 21.1 MiB IBGE asset installs to a verified 67.6 MiB read-only SQLite database. A separate regional flow plans and acquires fixed Copernicus GLO-30 DSM and WorldCover COGs plus tiny opt-in OSM `building`/`building:part` way responses using fixed HTTPS/same-origin routing, 2,048-character initial/redirect URL limits, a 384 MiB plan ceiling, source caps, provenance-valid resumable GET staging, SHA-256, path-keyed inventory schema 2/bounded v1 migration with strict completion provenance, a 24-hour verified live cache with force refresh, application-wide in-process repository serialization, bounded TIFF metadata indexing, and deterministic building GeoJSON with actual endpoint/upstream timestamp/raw tags. A non-executing job foundation adds canonical E6 plans, dual fingerprints, strict state/CAS, bounded private records, and pure reconciliation decisions. | Regional execution is still in-app and screen-bound; repository serialization is not multi-process and the job foundation is not wired to it. No scheduler/shared runner/process recovery, append-only snapshot history/pins, bare-earth DTM, raster sampling, terrain profile, RF clutter mapping, interpreted building heights, relations/holes, arbitrary import/removal, garbage collection, sector polygons, exact containment, map integration, population-by-coverage, API 23 regional runtime proof, or approved IBGE public redistribution exists. |
...
| JVM tests | Delivered baseline | The current 364 tests pass with no failures or skips. Regional additions cover fixed-source planning, transfer/cache/inventory/processing, cross-instance serialization and provenance hardening, canonical semantic/execution golden identities, job/license/state invariants, atomic store/CAS faults, and pure reconciliation decisions alongside the prior domain/RF/geographic/persistence/project/form/language coverage. | The job tests do not prove scheduler, notification, process/reboot recovery, or UI observation. A desktop semantic-fixture runner, terrain/LOS/Fresnel-clearance goldens, property testing, accessibility, performance, export, transformer fault automation, and complete system-flow coverage remain. |
| Instrumented tests | Delivered baseline | The complete connected suite contains 72 tests with no failures or skips on the Android 16/API 36 `Medium_Phone_API_36.1` emulator at system font scale 1.30. It includes 3 regional Catalog cases, all 5 `StudiesScreenTest` cases, 1 real-storage indexed schema-4-to-5 migration/integrity/reopen test, and 1 real-`AtomicFile` regional-job test. The preceding 18-test revision passed on the physical Android 16 reference phone. | A fresh physical run, API 23 dataset execution, true system-reclaim process termination, rotation/device matrix, broader accessibility automation, and CI execution remain. |
...
- durable-job execution beyond the delivered contract/store foundation, recovery/export UX, garbage collection, and the long-term operational-store decision;
...
| **G2 — Application architecture** | UDF, ViewModel, Navigation 3, dependency assembly, error model, restoration, and observability demonstrated. | **Foundation:** explicit actions/effects, structured recovery, use cases, injected dispatchers, typed saved-state route tests, and a pure regional job/state/reconciliation contract exist. Feature splitting, DI/scoping policy, runner/scheduler/UI integration, observability, accessibility, and true process-death/device flows remain. | Scaling feature count safely. |
...
- CI workflow, 364 passing JVM tests, lint with 0 errors and 12 dependency/tooling warnings, and debug APK/test APK;
- a complete green 72-test Android 16/API 36 emulator baseline for navigation, saved state, mutation completion, accessibility, draft protection, persisted project lifecycle, deterministic selection, Add RF Path, compact RF assets, Engineering Map, the real IBGE package, compact IBGE/regional Data Catalog behavior, 5 project-link Studies cases, 1 real-storage schema-4-to-5 migration case, and 1 real-`AtomicFile` regional-job case at system font scale 1.30; only the preceding 18-test revision is proven on the physical reference phone;
...
- regional job runner/scheduler/UI integration beyond the delivered job model/store/reconciler, sanitized diagnostics, correlation, and broader observability;
...
- durable-job execution/retention/migration beyond the delivered bounded per-record store, multi-process policy, Android `AtomicFile` interruption, and storage-exhaustion system evidence.
...
**State:** Geographic coordinate viewport, durable site move, bounded offline IBGE attributes, fixed-source regional raw-data acquisition/processing, and a non-executing regional job contract/store/reconciler foundation are delivered. Path-keyed inventory schema 2, bounded schema-1 migration, and explicit live-snapshot caching are included; offline basemap, bare-earth DTM/raster sampling, append-only snapshot ownership, general packages, scheduled/process-durable execution, and sector geometry remain planned.
...
- the durable lifecycle selected in ADR 0001: persist before enqueue, API 34+ Android [user-initiated data transfer jobs](https://developer.android.com/develop/background-work/background-tasks/uidt), API 23-33 constrained foreground [WorkManager long-running workers](https://developer.android.com/develop/background-work/background-tasks/persistent/how-to/long-running), one retry owner, one artifact at a time, notifications, cancellation, checkpoints, and process/reboot reconciliation;
...
| Durable regional jobs | Delivered foundation evidence: canonical E6 semantic/execution goldens, exact accepted licenses, provider attempt ceilings, checkpoint promotion plus future-artifact rejection, record-and-canonical-artifact contextual auditing of terminal and nonterminal committed outcomes, guarded non-mutating invalid-terminal reports, scheduler generations, cancel-first revision/fingerprint/expected-record-generation guards separated from concrete target kind/generation/identity, physical-target uniqueness, deterministic cancellation of extra stale/current targets in a bounded complete snapshot, unreadable-ID scheduler-entry preservation, expected-record-absent cancellation guards, typed generation-exhaustion orphaning, idempotent create, overlapping-path rejection, unreadable-record fail-closed ownership, single-winner CAS, real `AtomicFile` rollback/backup recovery, corrupt/future/oversized isolation, active/finished scheduler reconciliation, and no success inference. Remaining exit evidence: atomic reconciliation executor and store re-read before external cancellation; Data-screen persist-before-enqueue; production checkpoint writing/inventory validators; API 23-33 WorkManager and API 34+ UIDT selection; unique scheduling; constraints; notification/cancel; one shared retry owner; process/reboot reconciliation; and strong-ETag partial recovery. |
...
| Dataset size causes storage/ANR failure | Abandonment or corruption | The bundled IBGE slice has a 67.6 MiB installed size plus 16 MiB preflight allowance. Regional plans are capped at 384 MiB with per-artifact/Overpass bounds, streaming I/O, disk preflight, and eligible GET resume. General packages, durable scheduling, ownership, reference-aware removal, and garbage collection remain. |
