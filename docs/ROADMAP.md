...
| Brazil broadcast and digital-TV study | Delivered bounded slices | The planning-reference overlay retains its 72-radial FM/DTV protected profiles, HRP shaping, explicit fallback, non-regulatory state, and explicitly revoked E(50,10) FM/TV cochannel/adjacent envelopes. A separate CPU-only DTV workflow keeps the project transmitter independent, samples acquired terrain for HNMT, derives a P.1546 `E(50,90)` protected contour, evaluates P.526-15 Deygout-Assis D/U at every protected-boundary radial, produces an operational coverage surface, and exports HTML/PDF/XLSX/KMZ evidence. | Filing readiness still requires a bare-earth DTM, current verified Basic Plan snapshot, complete contour/D/U gates, independent numerical validation, and legal review. Strict FM interference, current-regulatory iso-field claims, durable executable study state, and complete desktop parity remain open. |
| JVM tests | Delivered evidence | The current aggregate discovered 602 tests: 601 passed, one Windows symlink-hardening case was permission-skipped, and there were zero failures or errors. The Android 16/API 36 emulator passed 99/99 tests with no failures or skips. `lintDebug` completed with 0 errors, 19 warnings, and 3 hints; debug APK and Android-test APK assembly succeeded. | The tests do not prove a production durable-job caller, late network-byte/checkpoint crash recovery, API 23/33 runtime behavior, process/reboot recovery, current-regulatory iso-interfering contour parity, or complete desktop parity. |
| Instrumented tests | Delivered evidence | The current Android 16/API 36 AVD aggregate passed 99/99 tests with no failures or skips, including arbitrary-element composer interaction, legacy-contour map semantics, and enlarged-text coverage render-mode/NoData reachability. A separate live official Anatel run downloaded and indexed 87,400 source records and exercised offline FM/TV query, source details, and page replacement. The preceding 18-test physical-phone revision remains historical evidence. | A fresh physical run, API 23 and API 33 scheduler execution, true system-reclaim process termination, reboot recovery, rotation/device matrix, broader accessibility automation, map performance, and CI execution remain. |
...
| MOB-033 | Add Brazil broadcast-contour reference plotting | Delivered bounded slice | P2 | MOB-012/013 | CPU-only source-pinned FM `E(50,50)` and derived first-generation DTV `E(50,90)` protected references, explicitly revoked FM/TV E(50,10) cochannel/adjacent envelopes, `E(80,80)` `NoData`, 72 radial pattern-shaped ERP, packaged-table provenance, distinct compact map states, and deterministic bounded KMZ export are delivered. The separate DTV study supplies current P.526-15 Deygout-Assis boundary D/U evidence. Strict FM interference, approved bare-earth terrain, independent parity, persisted executable studies, and filing approval remain open. |
